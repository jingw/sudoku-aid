            ...
            for (const [wr1, wc1] of settings.cellVisibilityGraph[pr][pc]) {
                ...
                for (const [wr2, wc2] of settings.cellVisibilityGraph[pr][pc]) {
                    ...
                    for (const rc of intersection) {
                        ...
                        if (board[r][c] & zMask) {
                            base.logRemoval(r, c, lowestDigit(zMask) + settings.startDigit - 1, `XY(Z) wing, pivot=${coordinateToStr(pr, pc)}, wings=${coordinateToStr(wr1, wc1)},${coordinateToStr(wr2, wc2)}`);
                            board[r][c] &= ~zMask;
                        }
                    }
                }
