...
export function eliminateObvious(settings, origBoard, board) {
 ...
 */
function intersectWithShift(board, coordinates, shift) {
...
export function eliminateFromConsecutiveKropkiDots(settings, origBoard, board) {
    ...
    for (const dots of settings.consecutiveKropkiDots) {
        ...
        for (let i = 0; i < dots.length; i++) {
            const [r, c] = dots[i];
            board[r][c] &= ((startAscending << i)
                | ((startDescending >>> i) & EMPTY_CELL));
        }
    }
}
export function shiftMultiply(set, factor) {
    let result = 0;
    for (let d = 1; d <= 9; d++) {
...
export function shiftDivide(set, factor) {
    let result = 0;
    for (let d = 1; d <= 9; d++) {
...
export function eliminateFromDoubleKropkiDots(settings, origBoard, board) {
    ...
    for (const dots of settings.doubleKropkiDots) {
        ...
        for (let i = 0; i < dots.length; i++) {
            const [r, c] = dots[i];
            board[r][c] &= (shiftMultiply(startAscending, 1 << i)
...
function forEachAssignment(bitSets, callback, used = 0, current = []) {
    ...
    for (const group of settings.groups) {
        ...
        for (let digit = 1; digit <= 9; digit++) {
            if (required & bitMask(digit)) {
                ...
                let intersectionOfVisibilities = null;
...
function coordinatesAsSet(coords) {
    const set = new Set();
...
function unionPossibilities(coords, board) {
    let union = 0;
    for (const [r, c] of coords) {
        union |= board[r][c];
    }
...
export function eliminateFish(_, origBoard, board) {
...
function tryClear(board, digit, r, c) {
    if (r >= 0 && r < 9 && c >= 0 && c < 9) {
        board[r][c] &= ~bitMask(digit);
    }
}
function clearOrthogonal(board, digit, r, c) {
    if (digit >= 1 && digit <= 9) {
...
function clearFrom(board, digit, r, c, settings) {
