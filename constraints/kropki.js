...
export class ConsecutiveKropkiDots extends Constraint {
    performElimination(_, origBoard, board) {
        ...
        const startAscending = intersectWithShift(origBoard, this.members, (s, i) => s >>> i);
        const startDescending = intersectWithShift(origBoard, this.members, (s, i) => s << i);
...
export function shiftMultiply(set, factor, boardSize, startDigit) {
...
export function shiftDivide(set, factor, boardSize, startDigit) {
    const offset = startDigit - 1;
    let result = 0;
    for (let d = 1; d <= boardSize; d++) {
...
export class DoubleKropkiDots extends Constraint {
    performElimination(settings, origBoard, board) {
        ...
        // get all possible values for first cell if ascending or descending
        const startAscending = intersectWithShift(origBoard, this.members, (s, i) => shiftDivide(s, 1 << i, board.length, startDigit));
        const startDescending = intersectWithShift(origBoard, this.members, (s, i) => shiftMultiply(s, 1 << i, board.length, startDigit));
        // apply to whole chain
        for (let i = 0; i < this.members.length; i++) {
            const [r, c] = this.members[i];
            board[r][c] &=
