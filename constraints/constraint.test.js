...
QUnit.test("BruteForceConstraint gives up with too many possibilities", (assert) => {
    const members = [];
    for (let i = 0; i < 8; i++) {
        members.push([0, i]);
    }
...
QUnit.test("BruteForceConstraint works with sufficiently few possibilities", (assert) => {
    const members = [];
    for (let i = 0; i < 2; i++) {
        members.push([0, i]);
    }
