import { SIZE_TO_BOX_COUNTS, packRC, unpackRC, } from "../sudoku.js";
import { NonRepeatingGroup, } from "./constraint.js";
import { EqualityConstraint } from "./equalities.js";
export function processSettings(settings) {
    const constraints = settings.constraints === undefined ? [] : [...settings.constraints];
    ...
    function buildBlockGroup(r0, c0, height, width, dr, dc) {
        const members = [];
    ...
    if (!settings.irregular && boardSize in SIZE_TO_BOX_COUNTS) {
        ...
        for (let R = 0; R < boxesTall; R++) {
            for (let C = 0; C < boxesWide; C++) {
                constraints.push(buildBlockGroup(R * boxHeight, C * boxWidth, boxHeight, boxWidth, 1, 1));
            }
        }
        if (settings.digitsNotInSamePosition) {
            for (let r = 0; r < boxHeight; r++) {
                for (let c = 0; c < boxWidth; c++) {
                    constraints.push(buildBlockGroup(r, c, boxesTall, boxesWide, boxHeight, boxWidth));
                }
            }
        }
    }
